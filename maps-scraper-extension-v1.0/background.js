  ...
  try {
    ...
    if (!urls.length) {
      ...
      state.status = finalBillingState.unlimited
        ...
        : `Done Ã”Ã‡Ã¶ ${state.results.length} listings. ${finalCredits} credits remaining.`;
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      state.status = currentBillingState.unlimited
        ...
        : `[${currentCredits} cr] Extracting ${i + 1} / ${urls.length}Ã”Ã‡Âª`;
      broadcast('STATE', state);
...
async function scrapeByClickingResults(tabId, maxResults) {
  ...
  while (state.running && successCount < maxResults && attempts < maxAttempts) {
    ...
    state.status = currentBillingState.unlimited
      ...
      : `[${currentCredits} cr] Scraping result ${successCount + 1} / ${maxResults}Ã”Ã‡Âª`;
    broadcast('STATE', state);
