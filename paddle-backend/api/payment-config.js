...
// - type=pricing: return public pricing mode/config (used by web + extension)
// - default: return checkout URL for payment page
// POST /api/payment-config
// - Admin only: update pricing mode
const { cors, paddleRequest, PADDLE_API_KEY, PADDLE_ENV, getRedis } = require('./_helpers');
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      console.error('Payment-config set mode error:', err);
  ...
  if ((req.query.type || '').toString() === 'pricing') {
  ...
  try {
    ...
    if (data?.data?.checkout?.url) {
      return res.status(200).json({
        ...
        mode,
        pricing: buildPublicPricing(mode),
      });
    }
